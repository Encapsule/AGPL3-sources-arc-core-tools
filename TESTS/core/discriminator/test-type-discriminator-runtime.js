...
genericFilterTestSuite({
    ...
    validConfig: false,
    expectedResults: {
        error: 'Filter [HnfY4Bk26YT-vsI4YL-giw::Discriminator Filter] failed while performing main operation. Unrecognized request format. Request signature must match one of filter set {[6UirSEewQLiM6VY_Uo1hSA::test1], [tKTz14sOR4OlXpBr_zZbdA::test3]}.',
        result: null
    }
});
...
genericFilterTestSuite({
    ...
    validConfig: false,
    expectedResults: {
        error: 'Filter [irGy2m4xQJhFgPGmyeexmg::Discriminator Filter] failed while performing main operation. Unrecognized request format. Request signature must match one of filter set {[0BN_zKZnSnmkhuhkFYTpCQ::test9], [1xEOwIdXT260V-M5zWYSNg::test10], [dyOir4HZSl2lBQCXqB2eWA::test11]}.',
        result: null
    }
});
...
genericFilterTestSuite({
    ...
    validConfig: false,
    expectedResults: {
        error: 'Filter [irGy2m4xQJhFgPGmyeexmg::Discriminator Filter] failed while performing main operation. Filter [dyOir4HZSl2lBQCXqB2eWA::test11] failed while normalizing request input. Error at path \'~.green\': Value of type \'jsUndefined\' not in allowed type set [jsString].',
        result: null
    }
});
